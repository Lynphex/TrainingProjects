create schema keepsql;

create table keepsql.coche(
...
create table keepsql.grupo(
	id serial primary key,
	nombre varchar(50)
);

create table keepsql.revision(
	id serial primary key,
	fecha_revision date,
	id_coche varchar(50),
	km_revision int,
...
create table keepsql.marca(
	id serial primary key,
	nombre varchar(50),
	id_grupo int
);

create table keepsql.modelo(
	id serial primary key,
	nombre varchar(50),
	id_marca int
);

create table keepsql.color(
	id serial primary key,
	nombre varchar(50)
);

create table keepsql.seguro(
	id serial primary key,
	id_coche varchar(50),
	id_aseguradora int,
	n_poliza int,
...
create table keepsql.aseguradora(
	id serial primary key,
	nombre varchar(50)
);

create table keepsql.moneda(
	id serial primary key,
	nombre varchar(50)
);

alter table keepsql.coche add constraint id_color_fk foreign key (id_color) references keepsql.color(id);
alter table keepsql.coche add constraint id_modelo_fk foreign key (id_modelo) references keepsql.modelo(id);
alter table keepsql.marca add constraint id_marcagrupo_fk foreign key (id_grupo) references keepsql.grupo(id);
alter table keepsql.modelo add constraint id_modelomarca_fk foreign key (id_marca) references keepsql.marca(id);
alter table keepsql.revision add constraint id_coche_fk foreign key (id_coche) references keepsql.coche(matricula);
alter table keepsql.revision add constraint id_moneda_fk foreign key (id_moneda) references keepsql.moneda(id);
alter table keepsql.seguro add constraint id_coche_fk foreign key (id_coche) references keepsql.coche(matricula);
alter table keepsql.seguro add constraint id_aseguradora_fk foreign key (id_aseguradora) references keepsql.aseguradora(id);

CREATE TABLE keepsql.datos_coches (
...
INSERT INTO keepsql.datos_coches (matricula,grupo,marca,modelo,fecha_compra,color,aseguradora,n_poliza,fecha_alta_seguro,importe_revision,moneda,kms_revision,fecha_revision,kms_totales) VALUES
...
insert into keepsql.grupo(nombre) select grupo from keepsql.datos_coches group by grupo;
insert into keepsql.marca(nombre,id_grupo)
select da.marca, g.id from keepsql.datos_coches da 
inner join keepsql.grupo g on da.grupo = g.nombre
group by marca, g.id;

insert into keepsql.modelo(nombre,id_marca)
select da.modelo, m.id from keepsql.datos_coches da
inner join keepsql.marca m on da.marca = m.nombre
group by modelo, m.id;

insert into keepsql.color(nombre) select color from keepsql.datos_coches group by color;
insert into keepsql.coche(matricula, fecha_compra, n_km_total, id_color, id_modelo) 
select da.matricula, da.fecha_compra, da.kms_totales, c.id, mo.id from keepsql.datos_coches da
join keepsql.color c on c.nombre = da.color
join keepsql.modelo mo on da.modelo = mo.nombre
group by matricula, fecha_compra, kms_totales, c.id, mo.id;

insert into keepsql.aseguradora(nombre) select aseguradora from keepsql.datos_coches group by aseguradora;
insert into keepsql.seguro(n_poliza, fecha_alta, id_coche, id_aseguradora)
select da.n_poliza, da.fecha_alta_seguro, co.matricula, ase.id from keepsql.datos_coches da
join keepsql.coche co on da.matricula = co.matricula
join keepsql.aseguradora ase on da.aseguradora = ase.nombre
group by n_poliza, fecha_alta_seguro, co.matricula, ase.id;

insert into keepsql.moneda(nombre) select moneda from keepsql.datos_coches group by moneda;
insert into keepsql.revision(fecha_revision, km_revision, importe, id_coche, id_moneda)
select da.fecha_revision, da.kms_revision, da.importe_revision, co.matricula, mon.id from keepsql.datos_coches da
join keepsql.coche co on da.matricula = co.matricula
join keepsql.moneda mon on da.moneda = mon.nombre
group by fecha_revision, kms_revision, importe_revision, co.matricula, mon.id;

select mo.nombre as modelo, ma.nombre as marca, gr.nombre as grupo, fecha_compra, matricula, col.nombre as color, n_km_total, se.n_poliza, ase.nombre as aseguradora
from keepsql.coche c
join keepsql.modelo mo on mo.id = c.id_modelo
join keepsql.marca ma on mo.id_marca = ma.id
join keepsql.grupo gr on ma.id_grupo = gr.id
join keepsql.color col on col.id = c.id_color
join keepsql.seguro se on se.id_coche = c.matricula
join keepsql.aseguradora ase on se.id_aseguradora = ase.id
